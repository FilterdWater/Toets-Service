...
export default function MakeExam({ exam }: MakeExamProps) {
    ...
    exam?.sections?.forEach((section) => {
        if (section?.new_page && currentPageSections && currentPageSections.length > 0) {
        ...
        <AppHeaderLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                ...
                {page > 0 && page < pages.length && (
                    <div className={`flex w-full ${page !== 1 ? "justify-between" : "justify-end"}`}>
                        {page !== 1 &&
                ...
                {page > 0 && pages[page - 1].sections?.map((section) => (
                    <div key={section.id} className="mb-8">
                        <Label className="text-xl mb-4">{section.name}</Label>

                        {section?.questions?.map((question) => (
                            <div key={question.id} className="mb-6">
                                <Label>{question.title}: {question.text}</Label>

                                {question.type === 'single_choice' &&
                                    question?.answers?.map((ans) => (
                                        <div key={ans.id} className="flex items-center gap-2 mt-1">
                                            <input
                                                type="radio"
                                                name={`q-${question.id}`}
                                                value={ans.answer_option}
                                                checked={answers[question.id] === ans.answer_option}
                                                onChange={() => handleAnswerChange(question.id, ans.answer_option)}
                                            />
                                    ...
                                    question?.answers?.map((ans) => (
                                        <div key={ans.id} className="flex items-center gap-2 mt-1">
                                            <input
                                                type="checkbox"
                                                value={ans.answer_option}
                                                checked={answers[question.id]?.includes(ans.answer_option) || false}
                                                onChange={() => handleAnswerChange(question.id, ans.answer_option, true)}
                                            />
                                            <span>{ans.answer_option}</span>
                                        </div>
                                    ))
